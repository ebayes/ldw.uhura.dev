                  ...
                  {'description' in model && (
                    <HoverCardContent side="right" align="start" className="w-[300px]">
                      <div className="flex items-center mb-1">
                        <Image 
                          src={(model as any).icon} 
                          alt={`${(model as any).provider || 'Model'} logo`} 
                          width={16} 
                          height={16} 
                          className="mr-1"
                        />
                        <div className="font-semibold text-sm">
                          <span className="text-gray-500">{(model as any).company || 'Unknown'}</span> / 
                          <span className="font-bold ml-1">{model.label}</span>
                        </div>
                      </div>
                      <div className="text-sm">{(model as any).description}</div>
                    </HoverCardContent>
                  )}
                </HoverCard>
              ))}
